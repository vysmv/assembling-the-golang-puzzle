...
>
> Для того чтобы команды, описанные в `Makefile`, корректно работали, я рекомендую использовать `WSL`, например, установить в нем `Ubuntu` и работать с проектом из этого Linux-окружения.
>
...
Далее мы можем либо самостоятельно выполнить эту миграцию или их множество (то есть таких пар файлов может быть сколько угодно), выполнив для этого:
...
Либо все невыполненные миграции запустятся автоматически при выполнении:
```bash
docker compose up
```

Как вы помните, в файле `docker/entrypoint.sh` есть проверка наличия файлов с миграциями, и если они есть, то запускается утилита `migrate`. Она выполнит все миграции, которые еще не выполнялись.
...
make migrate-down
```
То есть выполнятся все `down`-файлы.
...
Что приведет к восстановлению данных из бэкапа.
