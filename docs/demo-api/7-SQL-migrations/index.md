...
> Важно!
>
> Для имен миграций обычно используют короткое описание изменения в формате `действие_объект`. 
> Например: `create_movies_table`, `add_email_to_users`, `add_movies_check_constraints`, `rename_name_to_title_in_movies`. 
> Главное, чтобы по имени миграции было понятно, какое изменение выполняет ее up-файл. 
> Оба файла одной миграции имеют одинаковое имя и отличаются только суффиксами `.up.sql` и `.down.sql`.
