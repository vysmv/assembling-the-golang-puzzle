...
В общем, я запишу одно-два видео, чтобы и вы оценили, нужно ли вам продолжение темы спецификации или нет.

Если интересно, то дайте знать об этом в комментариях.

Ну хорошо, с этим всё, и давайте начнём.

## Введение / Introduction

А начнём мы с введения.  
Первое, что я вынес из спецификации, — это формальное определение языка. Думаю, это стоит вынести отдельно, так как именно здесь зафиксировано то, чего от языка следует ожидать.
...
> Иначе говоря, это язык, на котором написана инструкция к Go.
...
`Syntax = { Production } .` — это значит, что вся спецификация (Syntax) — это список повторяющихся правил (Production).  
Фигурные скобки `{}` означают, что правил может быть сколько угодно (от 0 до *n*).  
По сути, это просто формальное описание того, что спецификация состоит из глав, именуемых `Production`.

(Слайд №3)

На мой взгляд, главное, что стоит вынести из этого описания, — это работа операторов. Давайте на них посмотрим:
...
Главный вывод для разработчика: (Слайд №4) 
когда мы увидим в спецификации строку вроде:
- SliceType = "[" "]" ElementType .

То мы понимаем её структуру:

1. Это `Production` (правило).
2. `SliceType` — это имя правила (или, иначе говоря, нетерминал).
3. `"["` и `"]"` — это терминалы (то есть пишем их в коде как есть).
4. `ElementType` — это ссылка на другое правило (нетерминал).
5. `.` в конце — это просто маркер конца правила.

Также при чтении спецификации важно понимать, является ли слово «конечным» (это терминал) или оно само состоит из других правил (это нетерминал).  
Я сделал такой вывод:

(Слайд №5)

- Если я вижу **CamelCase** (например, `SliceType`) — это нетерминал (ссылка на другой Production).

- Если я вижу **lowercase** (например, `identifier`) — это терминал.  
  Это имена правил для базовых элементов, таких как идентификаторы (`identifier`), целые числа (`int_lit`) или операторы.  
  Но важно понимать, что мы не пишем в коде слово «identifier».  
  Вместо этого мы пишем любое имя (например, `myVar`).

- Далее **"текст" в кавычках** — это терминал.  
...
И в завершение — нюанс про маркеры версий (`[Go 1.xx]`).  
...
- Как один готовый символ (одна «кодовая точка»). Это так называемые *precomposed* символы (предварительно составленные). Они представляют один code point для `a` + `¨` — `U+00E4` (ä).
...
**Из этого разработчик может вынести следующий смысл:** если строка или идентификатор будут содержать предварительно составленный символ, а после при сравнении или обращении по идентификатору будет использован комбинированный символ, то компилятор выдаст ошибку, так как для него это два разных символа.
...
Где `s1` — это символ `ä`, представленный как одна кодовая точка (`U+00E4`).
...
Первым делом тут выполняется проверка наличия аргумента командной строки, то есть
...
Всё это реализовано как структура состояния, функция для создания значения и три метода:

- `Next`
- `Prev`
- `Open`

Вызов этих методов изменяет текущее состояние.

При этом `Domain` также ничего не знает за рамками своей зоны ответственности. 
Это чистая логика изменения состояния.
...
`App` — это слой приложения, или «оркестратор».
