...
Я поделюсь с вами своими заметками по чтению спецификации.

После того как я познакомился с основними концепциями и понял как работать с документацией я немного задумался, куда дальше идти? Можно ворваться в практику.
Это почти всегда хороший вариант. 
Но меня заинтересовала спецификация. 
Мне захотелось взять паузу перед практическим опытом и вникнуть в язык. 
Сделать это один раз и попытаться понять и принять то как устроен этот язык.   

В общем, я запишу пару видео, чтобы вы оценили нужно вам продолжение или нет.

Если да, то подписывайтесь на канал при просмотре этой серии. А я буду ориентироваться на кол-во подписчиков с этих двух серий.   

Ну хорошо, с этим все и вернемся к теме. 
...
> Образно, это язык на котором написана инструкция к Go.
...
Потом мы распечатываем 	результат сравнения s1 и s2, длинну строк s1 и s2 в байтах, и потом просто выводим значение s1 и s2 в терминал.
...
И если вдруг возникает такая ситуация, то выяснить в чем причина можно через разницу строк в байтах. То есть для Go символы это именно кодовые точки Unicode и не важно как символы выглядят на печати, а также то что ответственность за корректность в этом вопросе лежит на программисте, а не на компиляторе.
...
В качестве дополнения поделюсь тем как я решал возникшую передо мной практическую задачу.
Все примеры и тезисы я демонстрировал в консольном слайдере. 
И это небольшое приложение мы сейчас и разберем. 
...
Как вариант, вы можете поставить видео на паузу и попробовать выполнить самостоятельно, а потом вернуться к моему решению. Понимать альтернативный взгляд всегда легче после того как вы сами пытались это сделать. 

Я решил поделить программу на четыре слоя:

1. Domain (логика навигации)
State + методы Next/Prev/Open.

2. Store (источник данных)
Интерфейс SlideStore: “сколько слайдов” и “дай текст слайда k”.
Реализация: DirStore (читает файлы 1..n).

3. UI (рендер + ввод)
Рендер рамки на размер терминала, отображение X/Y, вывод текста/ошибки.
Ввод строкой через Enter (как в ТЗ).

4. App (оркестратор)
Цикл: render → read cmd → update state → repeat.

И сделать следующую структуру проекта
slider/
  go.mod
  cmd/slider/main.go
  internal/app/app.go
