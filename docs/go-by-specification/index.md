...
Но, кажется, что нам, по сути, нужно запомнить только результат этого описания — то есть работу операторов, давайте на них посмотрим:
...
(Слайд №4)
- **CamelCase** (например, `Type`) — это нетерминал (ссылка на другой Production).
...
(Слайд №6)
- Как один готовый символ (одна «кодовая точка»). Это так называемые *precomposed* (предварительно составленные): где один code point для `ä` — `U+00E4`.
...
**Для разработчика можно вынести следующий смысл:** для Go это разные символы. Если вы назовёте переменную первым способом, а попытаетесь обратиться к ней в коде, введя имя вторым способом, компилятор выдаст ошибку, так как для него это два разных идентификатора.

**В общем, стоит запомнить:** визуальное сходство знаков не означает их программную идентичность. Мы должны быть уверены, что используем одинаковый ввод.
...
### 3. Важный нюанс: Правила формирования идентификаторов  
